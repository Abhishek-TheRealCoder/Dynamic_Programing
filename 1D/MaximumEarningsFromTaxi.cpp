...
// you are going, and you want to drive from point 1 to point n to make money by picking up passengers. You cannot change the 
...
ll solve(int idx,int n,vector<vector<int>>& rides)
{
    if(idx >= n) return 0;
    if(dp[idx] != -1) return dp[idx];

    ll exclude = solve(idx + 1, n , rides);

    int s = idx + 1;
    int e = n - 1;
    int currElem = rides[idx][1];
    int startIdx = n;

    while(s <= e)
    {
        int mid = s + (e - s)/2;
        if(rides[mid][0] >= currElem)
        {
            startIdx = mid;
            e = mid - 1;
        }
    ...
    ll include = rides[idx][1] - rides[idx][0] + rides[idx][2] + solve(startIdx,n,rides);
...
int main()
{
    ...
    cout<<maxTaxiEarnings(n,rides)<<endl;
    return 0;
}