...
int solve(int i,int j,vector<int>&piles)
{
    ...
    int alice_takes_i = piles[i] + min(solve(i+1,j-1,piles),solve(i+2,j,piles));
    int alice_takes_j = piles[j] + min(solve(i+1,j-1,piles),solve(i,j-2,piles));

    return dp[i][j] = max(alice_takes_i,alice_takes_j);
}
bool stoneGame(vector<int>& piles) {
    int n = piles.size();
    memset(dp,-1,sizeof(dp));
    int first = solve(0,n-1,piles);
    int second = accumulate(piles.begin(),piles.end(),0) - first;
    return first > second;
}
